...
## 基于指定的本地分支创建新分支
git checkout -b new_branch local_existed_branch
# 以上都是在已有分支提交历史基础上创建新的代码分支，不会跟踪远端分支
# 如想跟踪远端分支使用下面git branch指令
## 基于远端存在分支创建新分支
git checkout -b new_branch origin/remote_existed_branch
# 跟踪远端分支的作用是可以在开发过程中实时拉取远端分支的更改
# 跟踪远端分支不能用于将新分支的更改直接推送到远端分支，git推送的逻辑是
# 如果远程仓库没有同名分支：创建新的远程分支
# 如果远程仓库有同名分支：推送到现有分支
# 如果新分支的更改已经推送，需要提交合并请求merge request才能合并到远端分支
...
```bash
# 将本地最新提交推送到远端分支
git push origin HEAD:remote-branch	# HEAD指向当前分支
git push [远程仓库名] [本地分支名]:[远程分支名]
# 推送当前分支到默认远程仓库，如果远程分支与本地分支不同名会失败
git push
# git推送的逻辑是
# 如果远程仓库没有同名分支：创建新的远程分支
# 如果远程仓库有同名分支：推送到现有分支
...
git config core.quotePath false		# git命令解析显示中文文件名
```
